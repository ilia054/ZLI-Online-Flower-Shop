...
/**
 * The class ApproveOrdersController is the Controller of "ApproveOrders" Screen
 */
public class ApproveOrdersController {
    ...
    private OrderStatus[] orderStatusArray = new OrderStatus[] {OrderStatus.APPROVED,  OrderStatus.NOT_APPROVED};
	...
	public void initialize() {
		...
		ZliClientUI.chat.accept(new Message(Task.Import_order_data_by_store, ZliClient.userController.getUser().getStore()));
		orders = ZliClient.orderController.getOrders();
		// Setting the Data to be displayed in the TableView
		OrderTable.setItems(orders);
		// Creating DropBox's for the color Column
		createDropBoxInCell(orderStatus);
		// Creating a Clock and Date Icons and their functionality
		/**
		 * This creates an "Event" when a User changes the selected color of the Order,
		 * We will save the change in the local memory of the TableView List
		 **/
	...
	/**
	 * This method terminates ApproveOrders scene and opens the StoreMangerMenu
    ...
    void saveClicked(MouseEvent event) {
		...
		Message message = new Message(Task.Update_orders_table_to_approved, orderChangedList);
		ZliClientUI.chat.accept(message);
		emails=StringController.getString();
		int size=emails.size();
		for (int i=0;i<size;i++) {
	    	StringBuilder str=new StringBuilder();
			mail.add(emails.get(i++).toString());
			str.append("Dear "+emails.get(i++).toString()+ " your order Number: "+emails.get(i++).toString()+"that you ordered at " +emails.get(i++) +" Has Been approved!\n at store:"+ emails.get(i).toString()+"\n Zli Store Manager "+ emails.get(i).toString());
